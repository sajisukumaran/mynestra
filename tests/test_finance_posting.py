...
def _opening(amount="1000"):
    """Dr Checking / Cr Opening Balance Equity — the canonical opening-balance entry."""
    return post_entry(
        date=JAN,
        description="Opening balance",
        entry_type=JournalEntry.EntryType.OPENING,
        lines=[
            LineInput(account="1120", debit=D(amount)),
...
def test_single_line_raises(make_tenant):
    ...
    with schema_context(tenant.schema_name):
        with pytest.raises(EmptyEntry):
            post_entry(date=JAN, lines=[LineInput("1120", debit=D("100"))])
    ...
    with schema_context(tenant.schema_name):
        with pytest.raises(InvalidLine):  # both sides
            post_entry(
                date=JAN,
                lines=[
                    LineInput("1120", debit=D("100"), credit=D("100")),
        ...
        with pytest.raises(InvalidLine):  # neither side
            post_entry(
                date=JAN,
                lines=[LineInput("1120"), LineInput("opening_balance_equity")],
            )
        with pytest.raises(InvalidLine):  # negative
            post_entry(
                date=JAN,
                lines=[
                    LineInput("1120", debit=D("-5")),
...
def test_entry_no_sequential_per_fiscal_year(make_tenant):
    ...
    with schema_context(tenant.schema_name):
        ...
        e2 = post_entry(
            ...
            lines=[LineInput("1120", debit=D("50")), LineInput("4100", credit=D("50"))],
        )
        ...
        e3 = post_entry(
            ...
            lines=[LineInput("1120", debit=D("10")), LineInput("4100", credit=D("10"))],
        )
...
def test_reverse_entry_mirrors_and_marks(make_tenant):
    ...
    with schema_context(tenant.schema_name):
        ...
        orig = entry.lines.get(account__code="1120")
        rev = reversal.lines.get(account__code="1120")
...
def test_idempotent_external_key(make_tenant):
    ...
    with schema_context(tenant.schema_name):
        kwargs = {
            ...
            "lines": [LineInput("1120", debit=D("75")), LineInput("4100", credit=D("75"))],
        }
...
def test_account_resolution_by_code_key_and_unknown(make_tenant):
    ...
    with schema_context(tenant.schema_name):
        entry = post_entry(
            date=JAN,
            lines=[
                LineInput("1120", debit=D("20")),  # by code
                LineInput("opening_balance_equity", credit=D("20")),  # by system_key
            ],
        )
        assert {line.account.code for line in entry.lines.all()} == {"1120", "3100"}
...
def test_post_time_guards_reject_header_and_inactive_accounts(make_tenant):
    ...
    with schema_context(tenant.schema_name):
        ...
        account = Account.objects.get(code="1130")
        account.is_active = False
        account.save(update_fields=["is_active"])
        with pytest.raises(InvalidLine):
            post_entry(
                date=JAN,
                lines=[LineInput("1130", debit=D("5")), LineInput("4100", credit=D("5"))],
            )
